"""
FCN Training - IMU-Only with Inter-Subject Split

This script trains an FCN using ONLY IMU data with SUBJECT-DISJOINT splitting:
- Only uses IMU data (48 channels from 8 sensors)
- Subjects are split into disjoint groups (no subject overlap across splits)
- This evaluates true generalization to unseen subjects
- Comparison with trial-level split shows effect of subject leakage
"""
...
def create_subject_disjoint_split(subjects, labels, train_ratio=0.70,
                                  val_ratio=0.15, seed=42):
    """Split subjects into disjoint train/val/test groups.
    
    Subjects are allocated to splits such that no subject appears in more than
    one split.  We sort subjects by their sample count and greedily assign them
    to the split that is furthest below its target size (a simple bin-packing
    heuristic that keeps sizes close to the desired ratio).
    
    Args:
        subjects: array of subject IDs per sample
        labels: array of labels per sample (for diagnostics)
        train_ratio: fraction of samples for training (~0.70)
        val_ratio: fraction of samples for validation (~0.15)
        seed: random seed
        
    Returns:
        train_idx, val_idx, test_idx: arrays of sample indices
        train_subj, val_subj, test_subj: sets of subject IDs per split
    """
    np.random.seed(seed)
    ...
    subj_counts = {s: int((subjects == s).sum()) for s in unique_subjects}
    
    # Shuffle subjects, then sort by count descending (greedy largest-first)
    ...
    for s in subj_list:
        cnt = subj_counts[s]
        # Compute how far each bin is below its target
    ...
    return train_idx, val_idx, test_idx, train_subj, val_subj, test_subj


def main():
    print("=" * 70)
    print("KneE-PAD FCN Training - IMU-ONLY with INTER-SUBJECT SPLIT")
    print("=" * 70)
    ...
    # ========================================================================
    # 3. Train/Val/Test Split - SUBJECT DISJOINT
    # ========================================================================
    print("\n[3/5] Splitting by SUBJECT (no subject overlap)...")
    print("  ✓ No subject appears in more than one split")
    
    train_idx, val_idx, test_idx, train_subj, val_subj, test_subj = \
        create_subject_disjoint_split(subjects, labels, seed=42)
    ...
    # Verify no overlap
    assert len(train_subj & val_subj) == 0, "Subject overlap train/val!"
    assert len(train_subj & test_subj) == 0, "Subject overlap train/test!"
    assert len(val_subj & test_subj) == 0, "Subject overlap val/test!"
    print(f"  ✓ Zero subject overlap across all splits")
    ...
    # Save normalization statistics (separate file for subject-split)
    models_dir = Path('models')
    models_dir.mkdir(exist_ok=True)
    np.savez(models_dir / 'fcn_imu_subject_normalization.npz',
             mean=imu_mean, std=imu_std)
    print(f"✓ Normalization stats saved to: models/fcn_imu_subject_normalization.npz")
    
    # Save subject split info for reproducibility
    np.savez(models_dir / 'fcn_imu_subject_split_info.npz',
             train_subjects=np.array(sorted(train_subj)),
             val_subjects=np.array(sorted(val_subj)),
             test_subjects=np.array(sorted(test_subj)))
    print(f"✓ Subject split info saved to: models/fcn_imu_subject_split_info.npz")
    ...
    # ========================================================================
    print("\n[5/5] Training FCN (IMU-Only, Subject-Split)...")
    ...
    for epoch in range(n_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), models_dir / 'best_fcn_imu_subject_split.pth')
    ...
    print(f"  Model saved to: models/best_fcn_imu_subject_split.pth")
    ...
    ax1.set_title('FCN Training Loss (IMU-Only, Subject-Split)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('FCN Training Accuracy (IMU-Only, Subject-Split)', fontsize=14, fontweight='bold')
    ...
    plt.savefig(vis_dir / 'fcn_imu_subject_split_training.png', dpi=300, bbox_inches='tight')
    print(f"✓ Plot saved to: {vis_dir / 'fcn_imu_subject_split_training.png'}")
    ...
    # Load best model
    model.load_state_dict(torch.load(models_dir / 'best_fcn_imu_subject_split.pth'))
    model.eval()
    ...
    plt.title('Confusion Matrix - FCN (IMU-Only, Subject-Split)', fontsize=14, fontweight='bold', pad=20)
    ...
    plt.savefig(vis_dir / 'fcn_imu_subject_split_confusion_matrix.png', dpi=300, bbox_inches='tight')
    print(f"✓ Confusion matrix saved to: {vis_dir / 'fcn_imu_subject_split_confusion_matrix.png'}")
    ...
    with open(results_dir / 'fcn_imu_subject_split_classification_report.txt', 'w') as f:
        f.write("FCN (IMU-Only, Subject-Split) - Classification Report\n")
        f.write("=" * 70 + "\n\n")
        f.write(f"Train subjects ({len(train_subj)}): {sorted(train_subj)}\n")
        f.write(f"Val   subjects ({len(val_subj)}):  {sorted(val_subj)}\n")
        f.write(f"Test  subjects ({len(test_subj)}):  {sorted(test_subj)}\n\n")
        f.write(classification_report(all_labels, all_preds, target_names=class_names, digits=3))
    print(f"✓ Classification report saved to: {results_dir / 'fcn_imu_subject_split_classification_report.txt'}")
    
    # ========================================================================
    # Final Results
    # ========================================================================
    print("\n" + "=" * 70)
    print("FINAL RESULTS - FCN (IMU-Only, Subject-Split)")
    print("=" * 70)
    print(f"Best Val Accuracy:   {best_val_acc:.2f}%")
    print(f"Final Test Accuracy: {test_accs[-1]:.2f}%")
    print(f"Architecture: FCN with {n_params:,} parameters")
    print(f"Data: IMU-only (48 channels)")
    print(f"Split: Subject-disjoint (70/15/15)")
